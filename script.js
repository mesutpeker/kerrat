...
function generateBalloons(correctAnswer) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        let leftPosition;
        do {
            leftPosition = Math.random() * 80 + 10; // 10% ile 90% arasında pozisyon
        } while (positions.some(pos => Math.abs(pos - leftPosition) < 20)); // Aralığı 20% yaparak artır
        ...
        if (i === correctPosition) {
            ...
            } while (number === correctAnswer || usedNumbers.has(number) || number < 0);
...
function startBalloonFall() {
    ...
    balloons.forEach(balloon => {
        ...
        delay += 1500; // Her balonun düşüşü arasında 1.5 saniye gecikme
    });
}
